using System.Windows;

namespace Smole
{
    #region Base

    /// <summary>
    /// A base class to run any animation method when a boolean is set to true
    /// and a reverse animation when set to false
    /// </summary>
    /// <typeparam name="Parent"></typeparam>
    public abstract class AnimateBaseProperty<Parent> : BaseAttachedProperty<Parent, bool>
        where Parent : BaseAttachedProperty<Parent, bool>, new()
    {
        #region Public Properties

        /// <summary>
        /// A flag indicating if this is the first time this property has been loaded
        /// </summary>
        public bool FirstLoad { get; set; } = true;

        #endregion

        public override void OnValueUpdated(DependencyObject sender, object value)
        {
            // Get the framework element
            if (!(sender is FrameworkElement element))
                return;

            // Don't fire if the value doesn't change
            if (sender.GetValue(ValueProperty) == value && !FirstLoad)
                return;

            // On first load...
            if (FirstLoad)
            {
                // Create a single self-unhookable event 
                // for the elements Loaded event
                RoutedEventHandler onLoaded = null;
                onLoaded = (ss, ee) =>
                {
                    // Unhook ourselves
                    element.Loaded -= onLoaded;

                    // Do desired animation
                    DoAnimation(element, (bool)value);

                    // No longer in first load
                    FirstLoad = false;
                };

                // Hook into the Loaded event of the element
                element.Loaded += onLoaded;
            }
            else
                // Do desired animation
                DoAnimation(element, (bool)value);
        }

        /// <summary>
        /// The animation method that is fired when the value changes
        /// </summary>
        /// <param name="element">The element</param>
        /// <param name="value">The new value</param>
        protected virtual void DoAnimation(FrameworkElement element, bool value) { }
    }

    #endregion

    #region Left

    /// <summary>
    /// Animates a framework element sliding it in from the left on show
    /// and sliding out to the left on hide
    /// </summary>
    public class AnimateSlideInFromLeftProperty : AnimateBaseProperty<AnimateSlideInFromLeftProperty>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.SlideAndFadeInFromLeft(FirstLoad ? 0 : 0.7f);
            else
                // Animate out
                await element.SlideAndFadeOutToLeft(FirstLoad ? 0 : 0.7f);
        }
    }

    // For Hamburger
    public class AnimateSlideInFromLeftPropertyHamg : AnimateBaseProperty<AnimateSlideInFromLeftPropertyHamg>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.SlideAndFadeInFromLeft(FirstLoad ? 0 : 0.3f);
            else
                // Animate out
                await element.SlideAndFadeOutToLeft(FirstLoad ? 0 : 0.3f);
        }
    }

    // For Grid Splitter
    public class AnimateSlideInFromLeftPropertySplitter : AnimateBaseProperty<AnimateSlideInFromLeftPropertySplitter>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.SlideAndFadeInFromLeft(FirstLoad ? 0 : 0.3f);
            else
                // Animate out
                await element.SlideAndFadeOutToLeft(FirstLoad ? 0 : 0.3f);
        }
    }

    #endregion

    #region Fade

    /// <summary>
    /// Animates a framework element fading in on show
    /// and fading out on hide
    /// </summary>
    public class AnimateFadeInProperty : AnimateBaseProperty<AnimateFadeInProperty>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.FadeIn(FirstLoad ? 0 : 0.3f);
            else
                // Animate out
                await element.FadeOut(FirstLoad ? 0 : 0.3f);
        }
    }

    // For Dimmed Overlay
    public class AnimateFadeInPropertyOverlay : AnimateBaseProperty<AnimateFadeInPropertyOverlay>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.FadeIn(FirstLoad ? 0 : 0.3f);
            else
                // Animate out
                await element.FadeOut(FirstLoad ? 0 : 0.3f);
        }
    }

    // New Group update
    public class AnimateFadeInPropertyGroupUpdate : AnimateBaseProperty<AnimateFadeInPropertyGroupUpdate>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.FadeIn(0.7f);
            else
                // Animate out
                await element.FadeOut(0);
        }
    }

    // Group Host
    public class AnimateFadeInPropertyGroups : AnimateBaseProperty<AnimateFadeInPropertyGroups>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.FadeIn(FirstLoad ? 0 : 0.3f);
            else
                // Animate out
                await element.FadeOut(0);
        }
    }

    // Resize Host
    public class AnimateFadeInPropertyResize : AnimateBaseProperty<AnimateFadeInPropertyResize>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.FadeIn(FirstLoad ? 0 : 0.3f);
            else
                // Animate out
                await element.FadeOut(0);
        }
    }

    // Resize Host
    public class AnimateFadeInPropertyBubble : AnimateBaseProperty<AnimateFadeInPropertyBubble>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.FadeIn(FirstLoad ? 0 : 0.3f);
            else
                // Animate out
                await element.FadeOut(FirstLoad ? 0 : 0.3f);
        }
    }

    #endregion

    #region Another


    /// <summary>
    /// Animates a framework element sliding it in from the right on show
    /// and sliding out to the left on hide
    /// </summary>
    public class AnimateSlideInFromRightProperty : AnimateBaseProperty<AnimateSlideInFromRightProperty>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.SlideAndFadeInFromRight(FirstLoad ? 0 : 0.7f);
            else
                // Animate out
                await element.SlideAndFadeOutToRight(FirstLoad ? 0 : 0.7f);
        }
    }

    /// <summary>
    /// Animates a framework element sliding it in from the left on show
    /// and sliding out to the left on hide
    /// </summary>
    public class AnimateSlideInFromBottomProperty : AnimateBaseProperty<AnimateSlideInFromBottomProperty>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.SlideAndFadeInFromBottom(FirstLoad ? 0 : 0.7f);
            else
                // Animate out
                await element.SlideAndFadeOutToBottom(FirstLoad ? 0 : 0.7f);
        }
    }

    /// <summary>
    /// Animates a framework element sliding it in from the left on show
    /// and sliding out to the left on hide
    /// </summary>
    public class AnimateSlideInFromTopProperty : AnimateBaseProperty<AnimateSlideInFromTopProperty>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await element.SlideAndFadeInFromTop(FirstLoad ? 0 : 0.7f);
            else
                // Animate out
                await element.SlideAndFadeOutToTop(FirstLoad ? 0 : 0.7f);
        }
    }

    /// <summary>
    /// Animates a framework element sliding it in from the top on show
    /// and sliding out to the left on hide
    /// </summary>
    public class AnimateSlideLoadProperty : AnimateBaseProperty<AnimateSlideLoadProperty>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            // Animate in
            await element.FadeIn(FirstLoad ? 0 : 0.3f);
        }
    }

    #endregion
}
